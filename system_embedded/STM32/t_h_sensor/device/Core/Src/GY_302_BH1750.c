...
response_bh1750 SDA_WaitAck(void)       //SLC高脉冲(窄)时读取SDA电平状态(宽脉冲)  ACK=0  NADC=1
{
...
response_bh1750 Send_measure(uint8_t command)               //BH1750写一个字节  
{
    SDA_Start();
    SDA_SendByte(BH1750_Addr|BH1750_WR);             //发送写地址
    if(SDA_WaitAck()==error1_bh1750)
        return error2_bh1750;                               //写入寻址错误
    SDA_SendByte(command);                           //发送控制命令
    if(SDA_WaitAck()==error1_bh1750)
        return error3_bh1750;                               //写入命令错误
    SDA_Stop();
    return ok_bh1750;
}
...
void Get_LIght_Intensity(void)          //获取光照强度*****************调用此函数
{   
...
//    switch(state)
//    {        
//        case error2_bh1750 :printf("error2 --> address+write error");break;              //寻址写入错误
//        case error3_bh1750 :printf("error3 --> write_command error");break;              //写入命令错误
//    }
    HAL_Delay(Wait_measure_Time);                                                 //等待测量完毕
    
    databuf=Read_data();                                                          //读取数据
    if(databuf!=error4_bh1750)
        
     light=(uint16_t)(databuf/1.2f*Resolurtion);                  //读取正常返回数值
//    
//               printf( "%d    lx\n",(uint8_t)(databuf/1.2f*Resolurtion));                  //读取正常返回数值

//           printf( "%d    lx\n",(uint16_t)(databuf/1.2f*Resolurtion));                  //读取正常返回数值
...
////response_bh1750 SDA_CheckDevice(uint8_t _Address)
////{
////    uint8_t checkAck;
////    SDA_Start();                                    /* 发送启动信号 */
