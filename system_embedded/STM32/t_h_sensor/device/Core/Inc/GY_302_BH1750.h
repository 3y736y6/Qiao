...
//可以使用////response_bh1750 SDA_CheckDevice(uint8_t _Address);对通讯接口进行检查而不触发采集数据和读取数据
...
#define SDA_PORT             GPIOB                             /* SCL所在PORT */
#define SCL_PIN              GPIO_PIN_6                        /* 连接到SCL时钟线的GPIO */
#define SDA_PIN              GPIO_PIN_0                        /* 连接到SDA数据线的GPIO */
#define SCL_PORT_ENABLE      __HAL_RCC_GPIOC_CLK_ENABLE();     /* SDA所在PORT使能 */
#define SDA_PORT_ENABLE      __HAL_RCC_GPIOB_CLK_ENABLE();     /* SDA所在PORT使能 */
/*----------------------------------------------------------------------------------------------------------*/
...
typedef enum  {ok_bh1750 = 0u, error1_bh1750,error2_bh1750,
                        error3_bh1750,error4_bh1750} response_bh1750;                       //error1 复位应答错误   error2 寻址+写 错误
...
response_bh1750 SDA_WaitAck(void);                                //等待应答
response_bh1750 Send_measure(uint8_t Measure_mode);               //寻址并发送测量模式
