...
from resnet_src1 import resnet34,resnet50,resnet101,resnet18
from torch.utils.tensorboard import SummaryWriter
from datetime import datetime  
from torch.utils.data import DataLoader, TensorDataset,random_split
import numpy as np
from MyDataset2 import MyDataset2
#########################   tensorboard --logdir=A1_fusion_model/runs   可视化查看指令
...
def main(my = True,train = True,test = True,color_channel = 1,data = 'STFT'):
    ...
    dataset = MyDataset2(data_file='bj_STFT2D_2channel_data.pkl', label_file='lable_5.csv')
    # dataset = MyDataset2(data_file='bj_STFT2D_data.pkl', label_file='lable_5.csv')
    if data == 'Wavelet':
        # dataset = MyDataset2(data_file='bj_wavelet_reshape128x128.pkl', label_file='lable_5.csv') # stft和小波数据集
        # dataset = MyDataset2(data_file='bj_wavelet_reshape128x1536.pkl', label_file='lable_5.csv') # stft和小波数据集
        dataset = MyDataset2(data_file='bj_wavelet_2channel_reshape128x1536.pkl', label_file='lable_5.csv') # stft和小波数据集
    class_numbers = 5
    classes = ('normal','broken','missing_tooth','root_crack','wear')

    # 划分比例
    # 生成相同的随机索引
    def get_random_split_indices(dataset, train_test_rate=0.6):
                # 设置随机种子
        seed = 19
    ...
    # 划分 dataset1
    train_indices, test_indices = get_random_split_indices(dataset)
    ...
    # 自定义模型
        net = resnet18(num_classes = 5,Data = data)
        print('自定义模型')
        color_channel = 1
    ...
    if train == True:
        ...
        summaryWriter = SummaryWriter(f"A1_fusion_model/runs/resnet/{formatted_datetime+data}")
        ...
        os.makedirs(os.path.dirname('A1_fusion_model/model/model_Resnet18.pth'), exist_ok=True)
        torch.save(net, 'A1_fusion_model/model/model_Resnet18.pth')
    ...
    if test == True:
        ...
        net = torch.load('A1_fusion_model/model/model_Resnet18.pth')  
...
if __name__ == '__main__':  # data='STFT' or data='Wavelet'
    main(my = True,train=True,test=True,data='Wavelet')