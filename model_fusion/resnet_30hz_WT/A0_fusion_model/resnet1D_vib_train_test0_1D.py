...
def main(my = True,train = True,test = True,data = '1D',repeat = 1):
    ...
    # dataset = MyDataset(data_file='bj_30hz_1channel_1D_data.csv', label_file='lable_5.csv')
    dataset = MyDataset(data_file='bj_30hz_2channel_1D_data.csv', label_file='lable_5.csv')
    ...
    for batch_idx, (inputs, labels) in enumerate(train_loader):  
        if batch_idx == 0:  # 只查看第一个批次  
            print("input shape:", inputs.float().unsqueeze(1).unsqueeze(1).repeat(1, 1, repeat, 1).shape)
            print("input type:", inputs.float().unsqueeze(1).unsqueeze(1).repeat(1, 1, repeat, 1).type())  
    ...
    if train == True:
        #绘制网络
        summaryWriter = SummaryWriter(f"A0_fusion_model/runs/resnet/{formatted_datetime+data}+1D")
        for i, (inputs, labels) in enumerate(train_loader):  
            if i == 1:  
                break  
            inputs, labels = inputs.float().unsqueeze(1).unsqueeze(1).repeat(1, 1, repeat, 1).to(device), labels.long().to(device)
        ...
        for epoch in range(epochs):
            ...
            for i, data in enumerate(train_loader):
                ...
                images = images.float().unsqueeze(1).unsqueeze(1).repeat(1, 1, repeat, 1)
        ...
        os.makedirs(os.path.dirname('A0_fusion_model/model/model_Resnet18_1D.pth'), exist_ok=True)
        torch.save(net, 'A0_fusion_model/model/model_Resnet18_1D.pth')
    ...
    if test == True:
        ...
        net = torch.load('A0_fusion_model/model/model_Resnet18_1D.pth')  
        ...
        with torch.no_grad():  # 测试集不需要反向传播
            for (images, labels) in test_loader:
                ...
                images = images.to(device)  
                images = images.float().unsqueeze(1).unsqueeze(1).repeat(1, 1, repeat, 1) # (batch_size,W,H)-->(batch_size,3,W,H)
                labels = labels.to(device)
                labels = labels.long()-1
...
if __name__ == '__main__':  # data='STFT' or data='Wavelet'
    main(my = True,train=True,test=True)